...
class Population:
    ...
    def update_live_players(self):
        # Find the best player (longest living)
        best_player = None
        max_lifespan = 0

        for p in self.players:
            if p.alive and p.lifespan > max_lifespan:
                max_lifespan = p.lifespan
                best_player = p

        # Update all live players
        for p in self.players:
            if p.alive:
                # Highlight the best player with a white outline
                is_best = (p == best_player)

                p.look()
                p.think()
                p.draw(config.window, is_best)
    ...
    def speciate(self):
        ...
        # Update species count in config
        config.species_count = len(self.species)
    ...
    def next_gen(self):
        ...
        self.generation += 1
        config.generation = self.generation  # Update global generation counter
