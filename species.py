...
class Species:
    def __init__(self, player):
        ...
        self.threshold = 1.2  # Default threshold
        ...
        # Adapt threshold based on current number of species (if available)
        if hasattr(config, 'target_species') and hasattr(config, 'species_count'):
            if config.species_count < config.target_species:
                # Decrease threshold to create more species
                self.threshold = max(0.8, self.threshold - 0.1)
            elif config.species_count > config.target_species:
                # Increase threshold to reduce number of species
                self.threshold = min(2.0, self.threshold + 0.1)
